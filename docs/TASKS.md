...
## T19. 관리 화면 목록형 전환 · 검색 · 레이아웃 정리

상태: 구현 완료 / CI 및 실제 UI 검증 대기

- [x] 월간 달력의 주 시작 요일을 일요일로 변경 (일~토)
- [x] 직원 관리를 목록 중심으로 변경
- [x] 직원 추가/수정을 dialog 레이어로 이동
- [x] 유일 관리자 보호 UI를 직원 수정 dialog에서도 유지
- [x] 연차 관리를 직원 목록 중심으로 변경
- [x] 연차 자동 발생/총 연차 고정/이월·조정/원장을 직원별 관리 dialog로 이동
- [x] 공휴일 관리를 목록 중심으로 변경
- [x] 공휴일 API 갱신/휴일 추가를 dialog 레이어로 이동
- [x] 변경 이력에 키워드 검색 + 작업 종류 필터 추가
- [x] 신청 내역에 키워드/상태/연도 검색 추가
- [x] 관리자 신청 내역 검색은 직원/부서까지 포함
- [x] 관리자 승인 화면 좌우 카드 높이/빈 공간 정렬 보정
- [x] 환경 설정 카드 그리드의 높이/폭 정렬 보정
- [x] 연차 관리 카드/원장 간격 및 modal 내부 영역 분리
- [x] 관리 목록 공통 toolbar/filter/table action 스타일 정리
- [x] PC/태블릿/모바일에서 목록/필터/modal 자동 재배치
- [x] PHP 8.2/8.4 CI
- [ ] 실제 브라우저에서 직원/연차/공휴일 dialog 열기·저장 검증
- [ ] 변경 이력/신청 내역 검색 조건 검증
- [ ] 관리자 승인/환경 설정의 대화면 카드 정렬 검증


## T20. 과거 휴가 날짜 진단 · 관리자 예외 기록

상태: 구현 완료 / CI 및 실사용 검증 대기

- [x] 근무일이 0건일 때 일반 문구 대신 원인 표시
- [x] 단일 날짜가 비근무 요일이면 날짜/요일/현재 근무요일 표시
- [x] 단일 날짜가 공휴일·회사 휴무일이면 등록된 휴일명 표시
- [x] 관리자에 한해 공가/병가/대체휴가 단일 날짜를 과거 기록 예외로 등록 가능
- [x] 예외 등록은 명시적 체크박스가 있어야 동작
- [x] 연차/반차처럼 연차를 차감하는 유형에는 예외 등록 불가
- [x] 예외 등록 여부를 Audit Log 상세에 기록
- [x] 2026-01-09이 기본 월~금 근무 설정에서 정상 근무일인지 회귀 테스트 추가
- [x] PHP 8.2/8.4 CI
- [ ] 실제 과거 대체휴가 등록 및 원인 메시지 검증


## T21. 관리자 대리 등록 즉시 승인

상태: 구현 완료 / CI 및 실제 통합 검증 대기

- [x] 관리자 승인 화면의 직원 대리 등록은 승인 대기 상태를 거치지 않음
- [x] 대리 등록 직후 기존 승인 서비스로 자동 승인
- [x] 연차/반차 대리 등록 시 승인과 동시에 연차 원장 차감
- [x] 대리 등록에서는 관리자 신청 알림을 중복 전송하지 않음
- [x] 신청자 개인 Telegram에 승인 결과 전송
- [x] 회사 공용 Telegram 그룹에 승인 일정 공유
- [x] Audit Log에 source=admin_proxy / automatic=true 기록
- [x] 초기 구현에서 잔여 연차 부족 경고를 구분했으며, T22에서 관리자 직접 등록은 잔여 연차 조건 자체를 적용하지 않도록 단순화
- [x] 관리자 대리 등록 버튼/안내 문구를 '등록 · 즉시 승인'으로 변경
- [x] PHP 8.2/8.4 CI
- [ ] 실제 연차/반차/G/S/A 대리 등록 즉시 승인 및 Telegram 검증


## T22. 관리자 직접 등록 · 월별 요약 · 연차 차감 표시

상태: 구현 완료 / CI 및 실제 UI 검증 대기

- [x] 관리자 직원 직접 등록은 선택한 날짜를 그대로 기록
- [x] 관리자 직접 등록 시 근무요일/공휴일/회사휴무 조건 미적용
- [x] 관리자 직접 등록 시 기존 pending/approved 중복 날짜 검사 미적용
- [x] 관리자 직접 등록 시 잔여 연차 조건/경고 미적용
- [x] 관리자 직접 등록 사유 선택은 선택사항
- [x] 관리자 직접 등록은 기존 승인 서비스로 즉시 approved 처리
- [x] 내 정보에 올해 1~12월 승인 휴가 월별 요약 추가
- [x] 월별 요약에서 연차 차감/미차감 일수 구분
- [x] 달력 데이터에 leave_types.deducts_annual_leave 포함
- [x] 달력 휴가 일정에 연차 차감/미차감 시각 구분
- [x] 달력 이달 신청 내역에 연차 차감/미차감 배지 표시
- [x] 휴가 상세 레이어에 승인 상태를 고려한 연차 차감 문구 표시
- [x] 관리자 직접 기간 등록용 exact calendar date range 테스트 추가
- [x] PHP 8.2/8.4 CI
- [ ] 실제 관리자 직접 등록(공휴일/주말/중복 포함) 즉시 승인 검증
- [ ] 내 정보 월별 요약 및 달력 차감 표시 브라우저 검증


## T23. 직원 · 연차 관리 통합

상태: 구현 완료 / CI 통과 / 실제 UI 검증 대기

- [x] 직원 관리와 연차 관리 화면 통합
- [x] 직원 목록에 연차 컬럼 추가
- [x] 연차 컬럼을 `사용 / 전체` 형식으로 표시
- [x] 연차 값 클릭 시 직원별 연차 관리 dialog 표시
- [x] 연도 변경, 자동 발생, 총 연차 고정, 이월·조정, 원장 조회 기능 유지
- [x] 연차 저장/조정 후 직원 관리 dialog로 복귀
- [x] 기존 `/admin/annual-leave` 접근은 직원 관리 화면으로 연결
- [x] 관리자 대시보드의 별도 연차 관리 메뉴 제거
- [x] PHP 8.2/8.4 CI
...
- [x] hotfix PR CI 통과
- [x] main 병합 및 main CI 통과
- [x] v1.3.1 GitHub Release 자동 생성 확인
...
- [x] 입사일 미등록 시 일반 신청 UI 및 달력 신청 진입 비활성화
- [x] 과거 날짜 포함 신청 시 이전 미기록 휴가 확인창 표시
- [x] 시작일 변경 시 종료일을 같은 날짜로 자동 동기화
- [x] 관리자 승인/반려 입력을 목록에서 dialog로 이동
- [x] 관리자 승인 휴가 취소 사유 입력을 dialog로 이동
- [x] 사용자 승인 휴가 취소 사유 입력을 dialog로 이동
- [x] 관리자 승인 목록 30건 제한 제거 및 전체 목록 제공
- [x] 주요 목록 10/20/50/100건 공통 페이징 적용
- [x] Telegram focus_request가 페이징 뒤 항목을 가리킬 때 해당 페이지 자동 선택
- [x] 달력 일정 카드 좌우 여백 및 내부 왼쪽 여백 개선
- [x] 달력 연도 2000 ~ 현재연도+3 제한
- [x] 최소/최대 연도 경계에서 이전/다음 달 이동 차단
- [x] 비근무 토요일 파란 계열 / 비근무 일요일 빨간 계열 배경
- [x] 근무 토·일 및 평일 비근무일 기존 규칙 유지
- [x] MariaDB 종료 휴가 정리 회귀 테스트 추가
- [x] README/요구사항/집계 문서 갱신
...
- [x] hotfix PR CI 통과
- [x] main 병합 및 main CI 통과
- [x] v1.4.2 GitHub Release 자동 생성 확인

## T35. v1.4.3 변경 이력 표 레이아웃 핫픽스

상태: 완료

- [x] 변경 이력 표 전용 고정 레이아웃 적용
- [x] 변경 이력 표 가로 스크롤 제거
- [x] 변경 이력 첫 열 sticky 해제
- [x] 긴 상세 데이터 셀 줄바꿈
- [x] 상세 key=value 항목별 자연스러운 wrapping
- [x] 모바일/좁은 화면 열 비율 보정
- [x] 변경 이력 안내 문구 중복 표현 정리
- [x] VERSION 1.4.3 및 릴리스 노트
- [x] hotfix PR CI 통과
- [x] main 병합 및 main CI 통과
- [x] v1.4.3 GitHub Release 자동 생성 확인
...
- [x] 관리자 직원별 통계
- [x] 관리자 부서별 통계
- [x] 선택 연도 연차 현황 및 순사용 소진율
- [x] 신청 단위 상세 기록
- [x] 실제 휴가일 단위 원본 기록
- [x] 데이터가 있는 연도별 상세 시트 자동 생성
- [x] 데이터가 있는 연월별 상세 시트 자동 생성
- [x] 관리자 전체 출력 시 직원별 상세 시트 자동 생성
- [x] 관리자 검토 필요 시트: 승인 대기 / 소급 입력 / 동일 일자 중복 승인
- [x] 신청 상태별 시각 스타일
- [x] 제목·설명·헤더·날짜·일수 형식 정리
- [x] 헤더 고정, 일부 첫 열 고정, 자동 필터, 병합 제목, 열 너비
- [x] 사용자 입력값 inline string 유지로 formula injection 방지
- [x] 외부 spreadsheet 라이브러리·추가 PHP 확장 없이 기존 OOXML writer 확장
- [x] 다중 worksheet ZIP/OOXML 구조 자동 테스트 추가
- [x] 모든 XML/relationship well-formed 검증 유지
- [x] MariaDB 기간 경계/사용자 범위/다중 시트 보고서 통합 테스트
