...
v1.1.0은 사용자 휴가 취소 흐름을 정리한 기능 릴리스다. 승인 전 신청 삭제, 승인된 휴가 사용자 취소, 연차 자동 복원, 취소 메타데이터 보존과 Telegram 알림을 MariaDB 통합 테스트로 검증한 소스를 배포한다. 아래 세부 항목은 향후 회귀 테스트와 운영 점검 기준으로 계속 유지한다.
