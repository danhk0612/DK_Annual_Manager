...
v1.5.0은 기존 휴가 Excel 내보내기를 통계·연차 현황·기간별/직원별 상세·검토 항목을 포함한 업무용 다중 시트 보고서로 확장한 기능 릴리스다.
...
- [x] XLSX 다중 worksheet / workbook relationship 구조 자동 테스트
- [x] XLSX 내부 XML/관계 파일 전체 파싱 테스트
- [x] Excel 기간 경계/사용자 범위 MariaDB 통합 테스트
- [x] Excel 실제 휴가일 원본 및 다중 시트 보고서 MariaDB 통합 테스트
...
- [ ] 사용자 보고서 요약/연도별/월별/휴가종류/연차 현황/신청 기록/일자별 기록 시트 확인
- [ ] 사용자 보고서 연도별·연월별 상세 시트 자동 분리 확인
- [ ] 관리자 전체/특정 사용자 × 연간/월간/전체 XLSX 다운로드 확인
- [ ] 관리자 직원별·부서별 통계 시트 확인
- [ ] 관리자 전체 출력 직원별 상세 시트 확인
- [ ] 관리자 검토 필요 시트에서 승인 대기/소급 입력/동일 일자 중복 승인 후보 확인
- [ ] Excel 헤더 고정/자동 필터/날짜·일수 형식/상태 스타일 확인
- [ ] Microsoft Excel에서 파일 복구 경고 없이 열리는지 확인
