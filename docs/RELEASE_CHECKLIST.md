...
v1.2.0은 Excel 내보내기와 달력 탐색 UX를 추가한 기능 릴리스다. 사용자/관리자 범위 및 연·월·전체 기간 필터, 실제 휴가 날짜 기준 집계, XLSX 패키지 구조, 달력 월 이동을 자동 테스트한 소스를 배포한다. 아래 세부 항목은 향후 회귀 테스트와 운영 점검 기준으로 계속 유지한다.
...
- [ ] XLSX ZIP/OOXML 구조 자동 테스트
- [ ] Excel 기간 경계/사용자 범위 MariaDB 통합 테스트
- [x] PHP 전체 구문 검사
