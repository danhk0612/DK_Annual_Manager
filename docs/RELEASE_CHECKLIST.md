...
v1.0.0은 자동 CI, MariaDB 통합 테스트, 운영 서버 `php bin/check.php --production`의 `FAIL 0 / WARN 0`, 실제 운영 smoke test 확인을 바탕으로 정식 안정 버전으로 배포한다. 아래 세부 항목은 향후 회귀 테스트와 운영 점검 기준으로 계속 유지한다.
