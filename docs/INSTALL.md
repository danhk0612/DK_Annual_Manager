...
- 사용자가 승인 전 신청을 취소·삭제하는 경우 별도 관리자 알림은 보내지 않는다.
- 사용자가 승인된 휴가를 취소하면 활성 관리자 개인 Telegram에 휴가 정보와 취소 사유를 알린다.
...
- 회사 공용 그룹은 관리자와 직원이 함께 보는 그룹으로, 승인된 휴가 일정 등록과 관리자/사용자에 의한 승인 일정 취소만 공유한다.


## 운영 중 재설치

활성 관리자는 **관리자 → 환경 설정 → 재설치 · 완전 초기화**에서 `RESET` 확인 후 모든 휴가관리 데이터를 삭제하고 설치 마법사부터 다시 시작할 수 있다.

관리자 계정에 접근할 수 없는 복구 상황에서는 서버의 PHP CLI에서 다음 명령을 사용한다.
...
기본값은 월~금이다. 설치 완료 후 **관리자 → 환경 설정 → 주 근무 요일**에서 월~일을 선택할 수 있다. 선택하지 않은 요일은 달력에서 비근무일로 표시되며 이후 생성되는 휴가 신청의 차감 날짜에서도 제외된다.
